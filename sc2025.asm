...
;;; 2025-03-18
;;; Erturk: Use as8 -octal -markascii -bin to assemble
;;;to ensure text has bit7 set to 1.
;;; JULY 2025: MGA created a UDF to Peak, Poke and execute with single function
;;; occupies regions identified as "patch" areas in book
;;; new labels UDFUP,UDFNEG,UDFPOS,INPUT6,INPUT5; redefines UDEFX
;;; This idea was thought about in 1970s at Scelbi, but not distributed


ENDPGRAM:  EQU 055       ;MGA 4/10/12 as in orig; for his ROMable Loboyko said 077       [077]
BGNPGRAM:  EQU 033       ;MGA 4/10/12 as in orig; for his ROMable Loboyko said 044       [044]
...
;;; just as suggested by Scelbal Manual for Serial I/O.
	...
	ORG 000#100		; save a bit of space before this
...
;;; 2025-03-18 Erturk
;;; RetroShield method of serial i/o
;;; CPRINT: Output character to OUTPORT - teensy will Serial.write() it.
;;; CINP: Read byte from INPORT
;;; - bit 7 = Serial.available() return value. 1=char available, 0=no char available.
;;; - bit 6..0 = character read by Serial.read() & 0x7F

;;; SCELBAL code is assembled with soft-uart routines.
;;; However Teensy code can override these routines w/
;;; direct drive CINP2 and CPRINT2 routines.
;;;   see scelbal.h for #define SCELBAL_USES_SOFT_UART 0

;CINP2:	INP INPORT          ; teensy will set bit7 = Serial.available()
;	NDA                        ; bit7 set?
;	JFS CINP                   ; no, keep waiting
;	OUT OUTPORT                ; echo
;	RET                        ; ascii char will be returned w/ bit7 set.
	
;CPRINT2: OUT OUTPORT        ; tell teensy to Serial.write() it.
;	RET
...
;;; Page one has many constants and variables.
	...
	ORG 001#000
	DATA *4
	DATA 000,000,100,001	; STORES FLOATING POINT CONSTANT +1.0
	DATA *3
	DATA 000		; EXPONENT COUNTER
	DATA 000,000,000,000	; STORES FLOATING POINT NUMBER TEMPORARILLY
	DATA *4
	DATA 000,000,300,001	; STORES FLOATING POINT CONSTANT -1.0
	DATA 000,000,000,000	; SCRATCH PAD AREA (16 BYTES)
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 001,120,162,002	; STORES RANDOM NUMBER GENERATOR CONSTANT VALUE
	DATA *4
	DATA 003,150,157,014	; STORES RANDOM NUMBER GENERATOR CONSTANT VALUE
	DATA 000,000,000,000	; SCRATCH PAD AREA (12 BYTES) (01 064-077)
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000		; SIGN INDICATOR
	DATA 000		; BITS COUNTER
	DATA 000,000		; SIGN INDICATOR
	DATA 000		; INPUT DIGIT COUNTER
	DATA 000		; TEMP STORATE
	DATA 000		; OUTPUT DIGIT COUNTER
	DATA 000 		; FP MODE INDICATOR
	DATA *7			; NOT ASSIGNED (SHOULD BE 01 111-117)
	DATA 000,000,000,000	; FPACC EXTENSION
	DATA 000,000,000,000	; FPACC LSW, NSW, MSW, EXPONENT
	DATA 000,000,000,000	; FPOP  Extension
	DATA 000,000,000,000	; FPOP  LSW, NSW, MSW, EXPONENT
	DATA 000,000,000,000	; FLOATING POINT WORKING AREA
	DATA 000,000,000,000	; (SHOULD BE AT 01 140-01-167)
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA *8			; NOT ASSIGNED (SHOULD BE 01 170-01 177)
	DATA 000,000,000,000	; TEMPORARY REGISTER STORAGE AREA (D,E,H&L)
	DATA *4			; NOT ASSIGNED (01 204-01 207)
	DATA 000,000,120,004	; STORES FLOATING POINT CONSTANT +10.0
	DATA 147,146,146,375	; STORES FLOATING POINT CONSTANT +0.1
	DATA 000		; GETINP COUNTER
	DATA *6			; NOT ASSIGNED (01 221-01 226)
	DATA 000		; ARITHMETIC STACK POINTER (01 227)
	DATA 000		; ARITHMETIC STACK (NOT CLEAR HOW LONG)
	ORG 001#272
	DATA 004		; CC FOR SAVE
	DATA "SAVE"
	DATA 004		; CC FOR LOAD
	DATA "LOAD"
	DATA 000,000,000,000	; UNCLEAR WHAT THIS IS (01 304-01 317) ZEROS
	DATA 000,000,000,000	; (PROBABLY STEP, FOR/NEXT, AND ARRAY PTR TEMP)
	DATA 000,000,000,000
	;; AT THIS POINT WE SHOULD BE AT LOCATION 01 320
	DATA 4
	DATA "THEN"
	DATA 2
	DATA "TO"
	...
	DATA 3
	DATA "SCR
	DATA 013		; CC FOR "READY" MESSAGE
	DATA 224,215,212	; CTRL-T, CARRIAGE RETURN, LINE FEED
	DATA "READY"
	DATA 215,212,212	; CARRIAGE RETURN, LINE FEED, LINE FEED;
	DATA 011
	DATA " AT LINE "
	;; THIS SHOULD BE THE END OF PAGE 01

	
	ORG 002#000		; START PAGE 02, THE CODE
	

SYNTAX:    CAL CLESYM             ;Clear the SYMBOL BUFFER area
           LLI 340                ;Set L to start of LINE NUMBER BUFFER
           LHI \HB\OLDPG26        ;** Set H to page of LINE NUMBER BUFFER
...
SYNTX4:    LLI 202                ;Set pntr to SCAN pntr stomge location
           ...
           LHI \HB\OLDPG27        ;** Set H to point to start of KEYWORD TABLE.
           LLI 000                ;Set L to point to start of KEYWORD TABLE.
SYNTX5:    LDI \HB\OLDPG26        ;** Set D to page of SYMBOL BUFFER
...
SYNTXL:    INL                    ;Pointers to get table address back and advance pntr to
           ...
           LHI \HB\OLDPG26        ;** And page of TOKEN in H. Fetch the value currently
...
SYNTX6:    LLI 202                ;Set L to SCAN pointer storage location
...
GETCHR:    LAM                    ;Get pointer from memory location pointed to by H&L
           ...
           LHI \HB\OLDPG26        ;** LINE INPUT BUFFER by setting up H too.
...
CLESYM:    LLI 120                ;Set L to start of SYMBOL BUFFER.
           LHI \HB\OLDPG26        ;** Set H to page of SYMBOL BUFFER.
...
CONCTS:    LLI 120                ;If character alphanumeric, can concatenate. Set pointer
           LHI \HB\OLDPG26        ;** To starting address of SYMBOL BUFFER.
...
CRLF:      LAI 215                ;Load ASCII code for carriage-return into ACC
           ...
           LHI \HB\OLDPG1         ;** Set H to page of COLUMN COUNTER
...
SCAN10:    LLI 200                ;Set L to the EVAL CURRENT pointer storage location
           LHI \HB\OLDPG26        ;** Set H to page of EVAL CURRENT pointer
...
PARSER:    LLI 120                ;Load L with starting address of SYMBOL BUFFER
           LHI \HB\OLDPG26        ;** Load H with page of SYMBOL BUFFER
...
NOEXPO:    LLI 227                ;Load L with address of ARITHMETIC STACK pointer
           LHI \HB\OLDPG1         ;** Load H with page of ARITHMETIC STACK pointer
           ...
           LHI \HB\OLDPG26        ;** Set H to page of the SYMBOL BUFFER
...
LOOKUP:    LLI 370                ;Load L with address of LOOK-UP COUNTER
           LHI \HB\OLDPG26        ;** Load H with page of the counter
           LMI 000                ;Initialize the counter to zero
           LLI 120                ;Load L with starting address of the SYMBOL BUFFER
           LDI \HB\OLDPG27        ;** Load D with page of the VARIABLES TABLE
...
LOOKU1:    LLI 121                ;Load L with addr of first character in the SYMBOL
           LHI \HB\OLDPG26        ;** BUFFER. Set H to page of the SYMBOL BUFFER.
...
LOOKU2:    CAL AD4DE              ;Call subroutine to add four to the pointer in D&E to
           ...
           LHI \HB\OLDPG26        ;** Up H and L to point to LOOK-UP COUNTER.
           ...
           LHI \HB\OLDPG27        ;** Do same for H. (Counts number of names in table.)
           ...
           LHI \HB\OLDPG27        ;** To table. First set H & L to the SYMBOL
           ...
           LHI \HB\OLDPG26        ;** BUFFER and set H to proper page. Set the number
...
LOOKU4:    CAL SAVEHL             ;Save current address to VARIABLES TABLE
           ...
           LHI \HB\OLDPG1         ;** Load H with page of the pointer
...
PARSE2:    LLI 210                ;Load L with address of the OPERATOR STACK pntr
           LHI \HB\OLDPG26        ;** Load H with page of the pointer
           LAM                    ;Fetch the stack pointer value to the accumulator
           ADL                    ;Add in the value of the stack pointer address to form
           LLA                    ;Address that points to top entry in the OS
...
FPOPER:    LLI 371                ;Load L with address of TEMP OP storage location
           LHI \HB\OLDPG26        ;** Load H with page of TEMP OP storage location
           LMA                    ;Store OP (from top of OPERATOR STACK)
           LLI 227                ;Change L to address of ARff HMETIC STACK pointer
           LHI \HB\OLDPG1         ;** Load H with page of AS pointer
           ...
           LHI \HB\OLDPG26        ;** Set H to page of TEMP OP storage location
...
PARNER:    LLI 230                ;If cannot find operator, expression is not balanced
           LHI \HB\OLDPG26        ;** Set H and L to address of F/A STACK pointer
...
INTEXP:    LLI 126                ;Load L with address of WSW of FPACC (Floating Point
           LHI \HB\OLDPG1         ;** ACCumulator). Load H with page of FPACC.
           ...
           LHI \HB\OLDPG1         ;** Set H to floating point working area page.
...
PRIGHT:    LLI 230                ;Load L with address of F/A STACK pointer
           LHI \HB\OLDPG26        ;** Load H with page of F/A STACK pointer
           ...
           LHI \HB\OLDPG27        ;** Location. Set H to page of F/A STACK TEMP
...
FUNAR1:    LLI 202                ;Load L with address of TEMP COUNTER
           ...
           LHI \HB\OLDPG26        ;** LOOK-UP TABLE. Set H to table page.
           CAL TABADR             ;Find address of next entry in the table
           LDI \HB\OLDPG26        ;** Load D with page of SYMBOL BUFFER
           ...
           LHI \HB\OLDPG27        ;** COUNTER and set H to the proper page. Fetch the
           ...
           LHI \HB\OLDPG27        ;** As well as H to the address of the TEMP COUI,.7ER
...
FUNAR4:    LLI 202                ;Load L with address of TEMP COUNTER
           LHI \HB\OLDPG27        ;** Set H to page of TEMP COUNTER
...
INTX:      LLI 126                ;Load L with address of MSW of the FPACC
           LHI \HB\OLDPG1         ;** Load H with the page of the PPACC
...
ABSX:      LLI 126                ;Load L with address of MSW of the FPACC
           LHI \HB\OLDPG1         ;** Set H to page of the FPACC
           LAM                    ;Fetch the MSW of the FPACC into the accumulator
...
SGNX:      LLI 126                ;Load L with address of MSW of the FPACC
           LHI \HB\OLDPG1         ;** Load H with the page of the FPACC
           LAM                    ;Fetch the MSW of the FPACC into the accumulator
...
CHRX:      CAL FPFIX              ;Convert contents of FPACC from floating point to
           ...
           LHI \HB\OLDPG26        ;** Set H to page of the TAB FLAG
...
STOSYM:    LLI 201                ;Load L with address of ARRAY FLAG
           LHI \HB\OLDPG27        ;** Load H with page of ARRAY FLAG
...
STOSY2:    LLI 121                ;load L with address of first character in the SYMBOL
           LHI \HB\OLDPG26        ;** BUFFER. Load H with page of the buffer.
...
STOSY3:    CAL AD4DE              ;Add four to pointer in registers D&E to skip over value
           ...
           LHI \HB\OLDPG26        ;** CNTR. Load H with page of TEMP CNTR.
           LBM                    ;Fetch the counter
           INB                    ;Increment the counter
           LMB                    ;Restore it to storage
           LLI 077                ;Set L to address of VARIABLES CNTR (indicates
           LHI \HB\OLDPG27        ;** Number of variables currently in table.) Set H too
           ...
           LHI \HB\OLDPG27        ;** VARIABLES CNTR. Set H too. Fetch the counter
           ...
           LHI \HB\OLDPG26        ;** Caracter in the SYMBOL BUFFER. Set H too.
...
SAVESY:    LLI 120                ;Load L with the address of the start of the SYMBOL
           LHI \HB\OLDPG26        ;** BUFFER. Load H with the page of the buffer.
...
RESTSY:    LLI 144                ;Load L with address of start of AUX SYMBOL BUFF
           LHI \HB\OLDPG26        ;** Load H with page of AUX SYMBOL BUFFER
...
EXEC1:     LLI 000                ;Load L with starting address of INPUT LINE BUFFER
           ...
           LDI \HB\OLDPG26        ;Load D with page of line input buffer
...
NOLIST:    LLI 342                ;Load L with address of RUN in look up table
           ...
           LDI \HB\OLDPG26        ;** If not RUN command, reset address pointers back
           ...
           LHI \HB\OLDPG26        ;** If found SCR command then load memory pointer
           ...
           LHI \HB\OLDPG27        ;** Storage location. Initialize this counter by placing
...
;;; This makes the SCR command clear the whole variable space, otherwise one space is lost.  
           ...
           LHI \HB\OLDPG57        ;@@ Load H with page of ARRAYS storage
...
NOSCR:     LEI 272                ;Load E with address of SAVE in look up table
           ...
           LHI \HB\OLDPG1         ;Up table and load H with page of look up table
           LDI \HB\OLDPG26        ;Load D with page of input line buffer
           ...
           LHI \HB\OLDPG26        ;** For USER PROGRAM BUFFER pointer. Initialize this
           ...
           LHI \HB\OLDPG26        ;** From SYNTAX subroutine set memory pointer to
...
GETAUX:    LLI 201                ;Set memory pointer to line character pointer storage
           LHI \HB\OLDPG26        ;** Location and then initialize that storage location
...
GETAU0:    LLI 201                ;Set memory pointer to line character pointer storage loc
           ...
           LHI \HB\OLDPG26        ;** Current string being built up in the auxiliary line
...
GETAU1:    LLI 201                ;Reset memory pointer to line character pntr storage loc
           LHI \HB\OLDPG26        ;On the appropriate page.
           LBM
...
GETAU2:    LLI 360                ;Reset mem pntr to pgm buffer line pntr storage
           LHI \HB\OLDPG26        ;** On this page and place the high order byte
...
PATCH3:	   LLI 201		; ptr to A/V storage
	   LHI \HB\OLDPG27      ; MGA 3/31/12 make relocatable; prev: LHI 027
	   LMI 000		; clear A/V flag
	   JMP EXEC
;;;
;;;  continuation of UDF (7/17/2025) by MGA uses the free space in this region
;;;
UDFPOS:
           LLI 372                ;low addr of earlier LMI instruction that modifies Lxx
           LMI 372                ;always restore self-mod inst back to LMC (normal)
                                  ;weird coincidence that above 2 bytes same
INPUT6:
           ;convert 8-bit acc to FP (MGA 7/15/2025) handy rtn for UDFs returning a byte
           LHI 001                ;page with FPACC
           LLI 123                ;FPACC extension
           LMI 000                ;has to be zero for convert to FP
           JMP INPUT5             ;convert byte to FP (reuse INPUT A$) and exit
...
NOTEND:    LLI 350                ;Load L with addr of auxiliary line number storage loc
           ...
           LDI \HB\OLDPG26        ;Load D with addr of line number buffer location
           ...
           LHI \HB\OLDPG26        ;** Line number from the user program buffer. Set up
           ...
           LHI \HB\OLDPG26        ;** Only of a line number by checking SYNTAX
...
NOSAME:    LLI 360                ;Reset memory pointer to program buffer
           LHI \HB\OLDPG26        ;Line pointer storage location
           ...
           LHI \HB\OLDPG26        ;** Do same for CPU register H
...
GETCHP:    LHI \HB\OLDPG26        ;** Load H with pointer page (low portion set upon
...
REMOV1:    LLI 364                ;Load L with end of user pgm buffer pointer storage loc
           LHI \HB\OLDPG26        ;** Load H with page of that pointer storage location
...
INCLIN:    LLI 000                ;Load L with start of line input buffer
           LHI \HB\OLDPG26        ;** Load H with page of start of line input buffer
...
FINERR:    LLI 340                ;Load L with starting address of line number storage area
           LHI \HB\OLDPG26        ;** Load H with page of line number storage area
...
NUMERR:    LAI 311                ;Set up ASCII code for letter 'I' in Accumulator
           ...
           LHI \HB\OLDPG1         ;** Routine. Do same for register H.
...
INSTR:     LDI \HB\OLDPG26        ;**Set D to starting page of LINE INPUT BUFFER
...
INSTR1:    CAL ADVDE              ;Advancer D & E pointer to the next location (input
           ...
           LHI \HB\OLDPG26        ;**Load H with page of input buffer.
...
RUN:       LLI 073                ;Load L with addr of GOSUB/RETURN stack pointer
           LHI \HB\OLDPG27        ;** Load H with page of same pointer
           ...
           LHI \HB\OLDPG26        ;** Load H with page of user pgm buffer line pointer
...
NXTLIN:    LLI 360                ;Load L with addr of user program buffer line pointer
           ...
           LHI \HB\OLDPG26        ;** Buffer pointer storage location. Store the new
           ...
           LHI \HB\OLDPG26        ;** To address of line number buffer. Fetch the last
...
SAMLIN:    LLI 360                ;Load L with addr of user program buffer line pointer
           LHI \HB\OLDPG26        ;** Load H with page of same pointer
           ...
           LDI \HB\OLDPG26        ;** Set D to page of line input buffer
           ...
           LHI \HB\OLDPG26        ;** Of the line input buffer.
...
DIRECT:    LLI 203                ;Load L with address of syntax TOKEN storage location
           LHI \HB\OLDPG26        ;** Load H with page of syntax TOKEN location
           ...
           LHI \HB\OLDPG26        ;@@ * * Set H to array pointer storage location.
...
PRINT:     LLI 202                ;Load L with address of SCAN pointer storage location
           LHI \HB\OLDPG26        ;** Load H with page of SCAN pointer
...
PRINT4:    CAL EVAL               ;Evaluate the current PRINT field
           ...
           LHI \HB\OLDPG26        ;** Load H with the page of the TAB flag
           ...
           LHI \HB\OLDPG1         ;** Change H to the FIXED/FLOAT flag page
...
PRINT5:    CTZ PFPOUT             ;If TAB flag not set, display value of expression
           ...
           LHI \HB\OLDPG26        ;** Load H with page of TAB flag
...
PRINT6:    LLI 203                ;Load L with address of PRINT pointer stomge location
           ...
           LHI \HB\OLDPG26        ;** Reset H to page of PRINT pointer stomge location
...
QUOTER:    LAI 311                ;If end of line before closing quote mark have an error
           ...
           LHI \HB\OLDPG26        ;** Load H with the page of the QUOTE flag
...
PFPOUT:    LLI 126                ;Load L with the address of the FPACC MSW (Floating
           LHI \HB\OLDPG1         ;** Point ACC). Load H with page of the FPACC MSW.
...
LET0:      CAL SAVESY             ;Entry point for IMPLIED LET statement. Save the
           ...
           LHI \HB\OLDPG26        ;** Pointer. Set H to the page of the SCAN pointer.
...
LET1:      LLI 202                ;Entry point for ARRAY IMPLIED LET statement.
           LHI \HB\OLDPG26        ;** Set pointer to SCAN pointer storage location
...
LET2:      LLI 203                ;Set L to gtorage location of LET pointer
           ...
           LHI \HB\OLDPG26        ;@@ ** Load H with page of ARRAY pointer
...
LET5:      LLI 203                ;When find the equal sign, reset L to point to the LET
           LHI \HB\OLDPG26        ;** Pointer and H to the proper page. Fetch the pointer
...
GOTO:      LLI 350                ;Load L with start of AUX LINE NR BUFFER
           LHI \HB\OLDPG26        ;** Load H with page of AUX LINE NR BUFFER
...
GOTO3:	   LLI 360                ;Set L to user program buffer pointer storage location
           LHI \HB\OLDPG26        ;** Set H to page of program buffer pointer
...
GOTO6:     LLI 204                ;Change L to the address of the GOTO SEARCH pointer
           LHI \HB\OLDPG26        ;** And H to the proper page of the pointer
...
GOTO7:     LLI 120                ;Load L with address of start of the SYMBOL BUFFER
           LHI \HB\OLDPG26        ;Set H to the page of the SYMBOL BUFFER
           LDI \HB\OLDPG26        ;Set D to the page of the AUX LINE NR BUFFER
           ...
           LHI \HB\OLDPG26        ;** Set H to page of user program buffer pointer
           ...
           LHI \HB\OLDPG26        ;** User program buffer. Place the new value for the user
...
IF:        LLI 202                ;Set L to SCAN pointer storage location.
           LHI \HB\OLDPG26        ;** Load H to page of SCAN pointer storage location.
           ...
           LHI \HB\OLDPG1         ;** Set H to page of the look-up table.
           ...
           LHI \HB\OLDPG27        ;** In the KEYWORD look-up table. Set H to table
...
IF1:       LLI 277                ;Load L with addr of EVAL FINISH pointer storage loc
           LHI \HB\OLDPG26        ;** Load H with page of storage location
           ...
           LHI \HB\OLDPG1         ;** Load H with page of FPACC MSW
           ...
           LHI \HB\OLDPG26        ;** Set H to the appmpriate page
...
GOSUB:     LLI 340                ;Load L with start of LINE NUMBER BUFFER
           LHI \HB\OLDPG26        ;Fetch (cc) of cuffent line number into register D
...
GOSUB1:    LLI 073                ;Set L to address of GOSUB STACK POINTER
           LHI \HB\OLDPG27        ;** Set H to page of GOSUB STACK POINTER
...
RETURN:    LLI 073                ;Set L to address of GOSUB STACK POINTER
           ...
           LHI \HB\OLDPG26        ;** Location. Load H with page of user pgm line pntr.
...
INPUT1:    LLI 203                ;Load L with address of INPUT pointer
           ...
           LHI \HB\OLDPG26        ;@@ ** Load H with page of array set up pointer
...
INPUT4:    CAL INPUTX             ;Get input from user for variable in SYMBOL BUFFER
           ...
           LHI \HB\OLDPG26        ;** Set H to page of INPUT pointer
...
INPUT5:    LLI 124                ;Load L with address of the LSW of the FPACC
...
FP0:       LHI \HB\OLDPG1         ;** Load H with floating point working registers page
...
FOR:       LLI 144                ;Load L with address of AUX SYMBOL BUFFER
           ...
           LHI \HB\OLDPG27        ;** Load H with page of FOR/NEXT STACK pointer
           ...
           LHI \HB\OLDPG26        ;** Set H to page of line pointer
           ...
           LHI \HB\OLDPG27        ;** Set H for page of the FOR/NEXT STACK
           ...
           LHI \HB\OLDPG1         ;** In a text strings storage area on this page
...
FOR1:      LLI 202                ;Load L with address of SCAN pointer storage location
           LHI \HB\OLDPG26        ;** Set H to page of the SCAN pointer
...
FOR4:      LLI 204                ;Set L with address of the FOR pointer
           ...
           LHI \HB\OLDPG26        ;** Load H with the page of the AUX SYMBOL BUFF
...
FPFIX:     LLI 126                ;Set L to point to MSW of FPACC
           LHI \HB\OLDPG1         ;** Set H to point to page of FPACC
...
FPNORM:    LAB                    ;Get CPU register B into ACC to check for special case
           LHI \HB\OLDPG1         ;** Set H to page of FPACC
...
FPADD:     LLI 126                ;Set L to point to MSW of FPACC
           LHI \HB\OLDPG1         ;** Do same for register H
...
CKSIGN:    LLI 140                ;Set pointer to start of partial-product working area
           LHI \HB\OLDPG1         ;** Set H to proper page
...
FLOAD:     LDI \HB\OLDPG1         ;** Set page address of FPACC
...
FSTORE:    LEL                    ;Transfer contents of register L to E
           ...
           LHI \HB\OLDPG1         ;** Set page to FPACC storage area
           JMP SETIT              ;Go transfer FPACC contents to area pointed to by D&E
OPLOAD:    LDI \HB\OLDPG1         ;** Set page to FPOP storage area
...
FACXOP:    CAL SAVEHL             ;Save contents of H & L upon entry to subroutine
           ...
           LHI \HB\OLDPG1         ;** Set pointer to page of FPACC
...
SAVEHL:    LAH                    ;Transfer value in H to ACC
           ...
           LHI \HB\OLDPG1         ;** And set H to storage area page
...
RESTHL:    LLI 200                ;Set L to start of temporary storage locations
           LHI \HB\OLDPG1         ;** Set H to storage area page
...
SWITCH:    LCH                    ;Transfer register H to C temporarily
           ...
           RET                    ;Exit to calling routine
GETINP:    LHI \HB\OLDPG1         ;** Set H to page of GETINP character counter
...
NOT0:      LLI 220                ;Set L back to address of GETINP counter which is used
           LHI \HB\OLDPG1         ;** As an indexing value. Set H to correct page.
           ...
           LHI \HB\OLDPG1         ;** Restore page pointer to floating point working area
...
DINPUT:    LEL                    ;Save entry value of register L in E. (Pointer to buffer
           ...
           LHI \HB\OLDPG1         ;** Set H to page of floating point working registers
...
FPX10:     LLI 210                ;Set pointer to registers containing floating point
           LHI \HB\OLDPG1         ;** Binary representation of 10 (decimal).
...
FPD10:     LLI 214                ;Set pointer to registers containing floating point
           LHI \HB\OLDPG1         ;** Binary representation of 0.1 (decimal).
...
FPOUT:     LHI \HB\OLDPG1         ;** Set H to working area for floating point routines
           LLI 157                ;Set pointer to decimal exponent storage location
...
DECEXT:    JFS DECEXD             ;If compensated exponent value is zero or positive
           ...
           LHI \HB\OLDPG1         ;** Set pointer to registers holding 10 (dec) in binary
...
DECEXD:    LLI 214                ;If exponent is positive, multiply FPACC by 0.1
           LHI \HB\OLDPG1         ;** Set pointer to registers holding 0.1 dec in binary
...
;;; NOW OPEN AREA UP TO 26 000 CAN BE USED FOR PATCHING...
	...
	ORG 026#000
	DATA 000		; CC FOR INPUT LINE BUFFER
	DATA *79 		; THE INPUT LINE BUFFER
	DATA 000,000,000,000	; THESE ARE SYMBOL BUFFER STORAGE
	DATA 000,000,000,000
	DATA 000,000,000,000	; SHOULD BE 26-120 TO 26 143
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000	; THESE LOCATIONS ARE AUXILIARY SYMBOL BUFFER
	DATA 000,000,000,000
	DATA 000,000,000,000	; SHOULD BE 26 144 TO 26 175
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000
	DATA 000		; TEMP SCAN STORAGE REGISTER
	DATA 000		; TAB FLAG
	DATA 000		; EVAL CURRENT TEMP REG.
	DATA 000		; SYNTAX LINE NUMBER
	DATA 000		; SCAN TEMPORARY REGISTER
	DATA 000		; STATEMENT TOKEN
	DATA 000,000		; TEMPORARY WORKING REGISTERS
	DATA 000,000		; ARRAY POINTERS
;;; NOW WE SHOULD BE UP TO 26 210
	DATA 000		; OPERATOR STACK POINTER
	DATA *15		; OPERATOR STACK
	DATA 000		; FUN/ARRAY STACK POINTER
	DATA *7			; FUNCTION/ARRAY STACK
...
;;; This SHOULD START AT 26 240
	...
	DATA 003		; PLUS SIGN
	DATA 003		; MINUS SIGN
	DATA 004		; MULTIPLICATION SIGN
	DATA 004		; DIVISION SIGN
	DATA 005		; EXPONENT SIGN
	DATA 006		; LEFT PARENTHESIS
	DATA 001		; RIGHT PARENTHESIS
	DATA 002		; NOT ASSIGNED
	DATA 002		; LESS THAN SIGN
	DATA 002		; Equal sign
	DATA 002		; GREATER THAN SIGN
	DATA 002		; LESS THAN OR EQUAL COMBO
	DATA 002		; EQUAL OR GREATER THAN
	DATA 002		; LESS THAN OR GREATER THAN
...
;;; This SHOULD START AT 26 257
	...
	DATA 004		; MULTIPLICATION SIGN
	DATA 004		; DIVISION SIGN
	DATA 005		; EXPONENTIATION SIGN
	DATA 001		; LEFT PARENTHESIS
	DATA 001		; RIGHT PARENTHESIS
	DATA 002		; NOT ASSIGNED
	DATA 002		; LESS THAN SIGN
	DATA 002		; EQUAL SIGN
	DATA 002		; GREATER THAN SIGN
	DATA 002		; LESS THAN OR EQUAL SIGN
	DATA 002		; EQUAL TO OR GREATER THAN
	DATA 002		; LESS THAN OR GREATER THAN

	DATA 000		; EVAL START POINTER
	DATA 000		; EVAL FINISH POINTER
...
;;; This SHOULD START AT 26 300
	...
	DATA 000,000,000,000	; LINE NUMBER BUFFER STORAGE
	DATA 000,000,000,000	; (SHOULD BE 340-347)
	DATA 000,000,000,000	; AUX LINE NUMBER BUFFER
	DATA 000,000,000,000	; (SHOULD BE 350-357)
...
;;; first time (later SCR commands load 33 into this spot) 
	DATA 033 		; USER PGM LINE PTR (PG)
	DATA 000 		; USER PGM LINE PTR (LOW)
	DATA 000 		; AUX PGM LINE PTR (PG)
	DATA 000 		; AUX PGM LINE PTR (LOW)
	DATA 000 		; END OF USER PGM BUFFER PTR (PG)
	DATA 000 		; END OF USER PGM BUFFER PTR (LOW)
	DATA 000		; PARENTHESIS COUNTER (366)
	DATA 000		; QUOTE INDICATOR
	DATA 000		; TABLE COUNTER (370)
;;; locations 371-377 NOT ASSIGNED
	...
	DATA 2
	DATA "IF"
	DATA 3    ;MGA 3/31/12 Loboyko transcription error
	DATA "LET"
	DATA 4
	DATA "GOTO"
	DATA 5
	...
	DATA 5
	DATA "GOSUB"
	DATA 6    ;MGA 4/12/12 another Loboyko transcription error
	DATA "RETURN"
	DATA 3
	DATA "DIM"
	DATA 3
	DATA "END"
	DATA 0			; END OF TABLE, SHOULD BE 072

	DATA 000		; GOSUB STACK POINTER
	DATA *1			; NOT ASSIGNED;
	DATA 000		; NUMBER OF ARRAYS COUNTER
	DATA 000		; ARRAY POINTER
	DATA 000		; VARIABLES COUNTER SHOULD BE 077
	DATA 000,000,000,000	; USED AS THE GOSUB STACK 100-117
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000	; USED AS ARRAY VARIABLES TABLE
	DATA 000,000,000,000	; SHOULD BE 120-137
	DATA 000,000,000,000
	DATA 000,000,000,000

	DATA 000,000,000,000	; USED FOR FOR/NEXT STACK STORAGE
	DATA 000,000,000,000	; SHOULD BE 140 TO 177
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000,000,000,000
	DATA 000		; FOR/NEXT STACK POINTER
	DATA 000		; ARRAY/VARIABLE FLAG
	DATA 000  		; STOSYM COUNTER
	DATA 000		; FUN/ARRAY STACK POINTER (203
	DATA 000		; ARRAY VALUES POINTER
	DATA *3			; NOT USED (SHOULD BE 205-207)
	DATA 000		; USED AS VARIABLES SYMBOL TABLE
	DATA *119		; (SHOULD BE 211-377 RESERVED)
...
NEXT:      LLI 144                ;Load L with start of AUX SYMBOL BUFFER
           LHI \HB\OLDPG26        ;** Set H to page of AUX SYMBOL BUFFER
...
NEXT3:     LLI 205                ;Load L with address of FOR/NEXT STACK pointer
           LHI \HB\OLDPG27        ;** Set H to page of FOR/NEXT STACK pointer
           ...
           LHI \HB\OLDPG27        ;** Set H to page of FOR/NEXT STACK
           LLA                    ;Move pointer value from ACC to CPU register L
           LDI \HB\OLDPG26        ;** Set register D to page of AUX SYMBOL BUFFER
...
NEXT4:     LLI 360                ;Load L with address of user program line pointer
           LHI \HB\OLDPG26        ;** Load H with page of user pgm line pntr storage loc.
           ...
           LHI \HB\OLDPG26        ;** Setting H & L to the user pgrn line pntr storage loc.
           ...
           LDI \HB\OLDPG26        ;** Change D to point to the line input buffer
           ...
           LHI \HB\OLDPG1         ;** Stored in a text strings storage area on this page
           CAL INSTR              ;Search the statement line for the occurrence of TO
           ...
           LHI \HB\OLDPG26        ;** Set H to page of EVAL pointer. Set up the starting
           LMA                    ;Position for the EVAL subroutine (after TO string)
           LLI 330                ;Set L to point to start of STEP string which is stored
           LHI \HB\OLDPG1         ;** In text stxings storage area on this page. Search the
           ...
           LHI \HB\OLDPG1         ;** To start of storage area for 1.0 in floating point
           ...
           LHI \HB\OLDPG26        ;** Set H to the page of the input line buffer
           ...
           LHI \HB\OLDPG1         ;** Load H with page of FOR/NEXT LIMIT registers
           CAL FSTORE  ;MGA 3/31/12 no lab here Store the FOR/NEXT LIMIT value
...
NEXT9:     LLI 202                ;Load L with address of SCAN pointer
           ...
           LLI 304                ;Load L with address of start of F/N STEP registers
           LHI \HB\OLDPG1         ;** Set H to page of F/N STEP registers
           ...
           LHI \HB\OLDPG1         ;**Set H to page of FIN TEMP storage registers
...
NEXT10:    LLI 363                ;If out of LIMIT range, load L with address of the AUX
           LHI \HB\OLDPG26        ;** PGM LINE pointer. (Contains pointer to the NEXT
...
NEXT12:    LLI 314                ;Load L with address of FIN TEMP storage registers
           LHI \HB\OLDPG1         ;** Set H to FIN TEMP storage registers page
...
FOR5:      LLI 205                ;Load L with address of the FOR/NEXT STACK pointer
           LHI \HB\OLDPG27        ;** Load H with page of the FOR/NEXT STACK pntr
           ...
           LHI \HB\OLDPG26        ;** AUX SYMBOL BUFFER and set up H to this page.
...
PARSEP:    LLI 176                ;Load L with PARSER TOKEN storage location. Set
           ...
           LHI \HB\OLDPG1         ;** Set H to the page of the ARITH STACK pointer.
...
;;; THERE were SOME BLANK ADDRESSES HERE 322-NEXT PAGE
;;; which MGA (7/12/2025) will use for the UDF; the UDF continues into other unused space elsewhere
UDEFX:           
           LAI 377                ;self-modify state >=1: prepare for exe, =0: exe, -1: normal
           NDA                    ;test state
           CFS UDFUP              ;if state <> -1 update state
           JTZ EXEC               ;sometimes self-modifying, use as storage for UDFPTR
                                  ;EXEC just something safe for jump to if UDFPTR not initialized 
                                  ;normally EXEC replaced with UDFPTR; jump triggered if state=0
           CAL FPFIX              ;otherwise (state=-1 or =1), onvert FPACC to fixed point
           LLI 124                ;FPACC LSW
           LCM
           INL                    ;FPACC NSW
           LBM                    ;16-big address if negative
           CAL FP0                ;zero FP argument; converted int unchanged in BC
           LLI 331                ;UDFPTR = pointer saved/used same HL both paths
           LHI 031                ;page address of target in above JTZ (UDFPTR)
           LAB                    ;NSW A
           NDA                    ;set sign flag
           JTS UDFNEG             ;if arg is neg, save address in UDFPTR; if pos use old UDFPTR
           LEM                    ;low UDFPTR byte
           INL
           LHM                    ;high UDFPTR byte
           LLE                    ;HL = UDFPTR
           LAM                    ;get new byte from mem indicated by pointer
           LMC                    ;self-modi  noop or (normal) store new byte in same address
           LHI 031                ;page of above self-modified instruction
           LLI 364                ;low addr of above instruction
           LMI 372                ;finish at UDFPOS to restore this self-mod inst LMC (normal)
           JMP UDFPOS             ; and to return acc as FP result  

	ORG 032#000
SQRX:      LLI 014                ;Load L with address of FP TEMP registers
           LHI \HB\OLDPG1         ;** Set H to page of FP TEMP. Move contents of FPACC
...
RNDX:      LLI 064                ;Load L with address of SEED storage registers
           LHI \HB\OLDPG1         ;** Set H to page for floating point working registers
...
PRIGH1:    LLI 126                ;Load L with address of the MSW in the FPACC
           ...
           LHI \HB\OLDPG27        ;** Load H with page of F/A STACK TEMP
           ...
           LHI \HB\OLDPG27        ;** TABLE to forin pointer. Set page address in H.
           ...
           LHI \HB\OLDPG57        ;tt The pointer to that location. Load the floating point
...
FUNAR2:    LLI 202                ;Load L with address of TEMP COUNTER
           LHI \HB\OLDPG27        ;** Load H with page of counter
           LBM                    ;Fetch the counter value
           INB                    ;Increment the value
           ...
           LHI \HB\OLDPG27        ;** TABLE (less four). Set H to page of the table.
           CAL TABADR             ;Calculate address of start of next narne in table.
           LDI \HB\OLDPG26        ;** Load D with page of the SYMBOL BUFFER
           ...
           LHI \HB\OLDPG27        ;** COUNTER. Set H to page of TEMP COUNTER.
...
ARRAY3:    LHI \HB\OLDPG26        ;** Set H to pointer page
...
ARRAY6:    LLI 207                ;Load L with address of LOOP COUNTER
           LHI \HB\OLDPG26        ;** Load H with page of LOOP COUNTER
           LBM                    ;Fetch the counter value
           INB                    ;Increment it
           ...
           LDI \HB\OLDPG26        ;** Set D to page of SYMBOL BUFFER
...
ARRAY7:    CAL EVAL               ;Call subroutine to evaluate subscript expression
           ...
           LHI \HB\OLDPG1         ;** Set H to page of subscript value
           ...
           LHI \HB\OLDPG27        ;** ARRAY VALUES table. Now set H & L to TEMP
...
DIM4:      LLI 206                ;Load L with address of ARRAY pointer storage loc
           LHI \HB\OLDPG26        ;** Set H to page of ARRAY pointer storage location
           ...
           LHI \HB\OLDPG27        ;** Address to form pointer to ARRAY VARIA.BLES
           LLA                    ;Table and set up H & L as the memory pointer.
           LEI 120                ;Load E with starting address of the SYMBOL BUFFER
           LDI \HB\OLDPG26        ;** Load D with the page address of the SYMBOL BUFF
           ...
           LHI \HB\OLDPG26        ;** Load H with page of ARRAY pointer storage
           LBM                    ;Fetch the ARRAY pointer value to register B
           INB                    ;Increment the value
           LMB                    ;Restore it to ARRAY pointer storage location
           LLI 075                ;Change L to number of arrays storage location
           LHI \HB\OLDPG27        ;** Set H to page of the number of arrays stomge loc
           ...
           LHI \HB\OLDPG27        ;** Append naine to table. First set pointer to number
           ...
           LDI \HB\OLDPG27        ;** Set register D to the page of the table.
           LLI 120                ;Load L with the start of the SYMBOL BUFFER
           LHI \HB\OLDPG26        ;** Load H with the page of the SYMBOL BUFFER
           ...
           LHI \HB\OLDPG26        ;** Location. Set H to the DIM pointer page.
...
DIM7:      LLI 120                ;Load L with address of start of SYMBOL BUFFER
           LHI \HB\OLDPG26        ;** Load H with page of SYMBOL BUFFER. (Now
           ...
           LHI \HB\OLDPG27        ;** Set H to ARRAY TEMP pointer page.
           ...
           LHI \HB\OLDPG27        ;Table and set the pointer up in registers H & L.
...
DIM8:      LLI 204                ;Set L to address of DIMEN pointer storage location
           LHI \HB\OLDPG26        ;** Set H to page of DIMEN pointer
...
;;; MGA (7/12/2025) continuation of UDF patch
UDFNEG:    
           XRI 377                ;high 8 complement
           LBA
           LAC                    ;low 8 complement
           XRI 377
           ADI 001                ;inc low 8
           LCA
           LAB
           ACI 000                ;inc high
           LMC                    ;store negated addr into UDFPTR
           INL
           LMA
           RET                    ;return 0.0 due to FP0 (so UDF(-addr)+UDF(posbyte) = old byte)
UDFUP:
           LHI 031
           LLI 323
           SUI 001
